...
export default function LoginPage() {
  // View State: 'login', 'forgot', or 'reset'
  ...
  useEffect(() => {
    ...
    // If URL has token and type=3, switch to Reset Password View
    if (token && type === '3') {
      setResetToken(token);
      setView('reset');
      // Clean up URL so the token isn't visible in address bar forever
    ...
    try {
      const response = await fetch(`${API_BASE}/reset_password`, {
        ...
        headers: { 
            ...
            "Authorization": `Bearer ${resetToken}` // Send Token from Email
        },
              ...
              <form onSubmit={handleLogin} className="space-y-6 relative z-10">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-800 pr-10"
                    required
                  />
                  <button
                    type="button"
